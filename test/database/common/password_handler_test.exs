defmodule Database.Common.PasswordHandlerTest do
  use Database.DataCase
    ...
    test "test put_password_hash with valid data" do
      admin_user_fixture()
      assert {:ok, %AdminUser{}} = PasswordHandler.authenticate_admin_user(@valid_attrs.email, @valid_attrs.password)
    end

    test "try put_password_hash with invalid data" do
      assert PasswordHandler.authenticate_admin_user(@invalid_attrs.email, @invalid_attrs.password) == {:error, :invalid_credentials}
    end
  end
end
